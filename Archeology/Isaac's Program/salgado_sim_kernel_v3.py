...
def compute_radial_gradient_consistency(grad_x, grad_y, radius):
    """f2: Radial Gradient Consistency - validates gradients point radially outward"""
    h, w = grad_x.shape
    consistency = np.zeros((h, w))
    
    for cy in range(radius, h-radius):
        for cx in range(radius, w-radius):
            # Extract local gradient patch
            ...
            # Create expected radial gradient pattern
            local_y, local_x = np.ogrid[:2*radius+1, :2*radius+1]
            dy = local_y - radius
            dx = local_x - radius
            dist = np.sqrt(dy**2 + dx**2) + 1e-6
            
            # Expected radial gradients (pointing outward)
            expected_gx = dx / dist
            expected_gy = dy / dist
            
            # Compute alignment between actual and expected gradients
